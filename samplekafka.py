...
parser.add_argument('--sample_ratio', default=0.5, type=float, help="fraction of sample")
...
def sample_kafka(from_server, from_topic, to_server, to_topic, sample_ratio=0.5):
...
if __name__ == '__main__':
    ...
    PIDFILE = '/tmp/phystats_sampk_daemon.pid'
    if args.daemon:
        if args.daemon_action == 'start':
            try:
                daemonizef(PIDFILE,
                        stdout='/tmp/phystats_sampk_daemon.log',
                        stderr='/tmp/phystats_sampk_daemon.log')
            except RuntimeError as e:
                print(e, file=sys.stderr)
                raise SystemExit(1)
            # 守护进程中运行的主程序
            sample_kafka(from_server, from_topic, to_server, to_topic, sample_ratio=args.sample_ratio)
